...
public class JeffChestSortOrganizer {
   boolean debug = false;
   String sortingMethod = "{category},{itemsFirst},{name},{color}";
	...
	public JeffChestSortOrganizer(ChestSorter plugin) {
		this.plugin = plugin;
	...
	String[] getTypeAndColor(String typeName) {
		...
		String myColor = (debug) ? "~color~" : emptyPlaceholderString;
	...
	CategoryLinePair getCategoryLinePair(String typeName) {
		...
		return new CategoryLinePair((debug) ? "~category~" : emptyPlaceholderString, (short) 0);
	}
	...
	String getSortableString(ItemStack item) {
		...
		String sortableString = sortingMethod.replaceAll(",", "|");
